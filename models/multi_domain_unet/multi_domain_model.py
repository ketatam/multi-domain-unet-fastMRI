...
class MultiDomainBlock(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, drop_prob):
        ...
        self.layers = nn.Sequential(
            ComplexConv2d(in_chans, out_chans // 2, kernel_size=3, padding=1, bias=False),
            ComplexInstanceNorm2d(out_chans // 2),
    ...
    def forward(self, input):
        ...
        image_output = self.layers(image)
        kspace_output = self.layers(kspace)
...
class TransposeConvBlock(nn.Module):
    ...
    def __init__(self, in_chans, out_chans):
        ...
        self.layers = nn.Sequential(
            ComplexConvTranspose2d(in_chans, out_chans // 2, kernel_size=2, stride=2, bias=False),
            ComplexInstanceNorm2d(out_chans // 2),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
